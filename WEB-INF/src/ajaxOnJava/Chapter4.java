...
public class Chapter4 extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        if (key != null) {
            ...
            switch(type){
				case "bufferRead" : res.getWriter().write( createStringBufferJSON(keyInt) );
				break;
				
				case "json" : res.getWriter().write( createJSONwithJSONsimple(keyInt) );
				break;	
					
				case "etc" : 
					// Simple API for XML(SAX)
					res.getWriter().write( createSAXXML(keyInt) );
				break;	
			}
	...
	// UsingStringBuffer
	private String createStringBufferJSON(int keyInt){
	...
	public String createJSONwithJSONsimple(int keyInt)
    {
		...
		obj.put("decimal",Integer.toString(keyInt));
		obj.put("hexadecimal",Integer.toString(keyInt,16));
		obj.put("octal",Integer.toString(keyInt,8));
		obj.put("hyper","&0x"+Integer.toString(keyInt,16));
		obj.put("binary",Integer.toString(keyInt,2)+"B");
...
//      serializer.setOutputProperty(OutputKeys.DOCTYPE_SYSTEM, "System ");
      ...
      // Start the document 문서 시작
      transformerHandler.startDocument();
      // list all the attributes for element 엘리먼트의 모든 속성 열기
      AttributesImpl attr = new AttributesImpl();
      // start writing elements, every start tag and end tag has to be defined 모든 시작 태그와 끝나는 태그는 명시적으로 정의 되어야 함.
      // explicitly.
